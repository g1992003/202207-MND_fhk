...
if (!isset($id) || !is_numeric($id)) {
	...
	$sql = "SELECT c_title_1,[a_category].c_id FROM [video] INNER JOIN [a_category] ON[video].c_id = [a_category].c_id WHERE v_id = :id";
	...
	if ($parents_id >= 15 && $parents_id <= 20) {
		$page_name = "video.php?id=" . $parents_id;
	}
...
<body data-color="grey" class="flat">
	<div id="wrapper">
		...
		$group_array = array(
			...
			array("input", "年份", $text_name, "", "1", ""),
			array("input", "連結", $url_name, "", "", ""),
			array("input", "圖片", $img_name, "", "", ""),
		);
		if ($parents_id <= 30) {
			$group_array = array(
				array("input", "標題《中文》", $title_name_1, "", "", ""),
	...
	<script type="text/javascript">
		<?php if ($parents_id != 15) { ?>
			const nameElement1 = document.getElementsByName('v_img');
